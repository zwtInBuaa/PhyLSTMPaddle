...
def train(cfg: DictConfig):
    ...
    ag_all = ag_data.astype(np.float32)
    u_all = u_data.astype(np.float32)
    u_t_all = ut_data.astype(np.float32)
    u_tt_all = utt_data.astype(np.float32)
    ...
    temp1 = np.concatenate([-1 / 2 * np.identity(N - 2), np.zeros([N - 2, 2])], axis=1).astype(np.float32)
    temp2 = np.concatenate([np.zeros([N - 2, 2]), 1 / 2 * np.identity(N - 2)], axis=1).astype(np.float32)
    phi2 = temp1 + temp2
    phi3 = np.concatenate(
        [
            np.zeros([N - 3]),
...
def evaluate(cfg: DictConfig):
    ...
    ut_data = mat["target_Xd_tf"]
    utt_data = mat["target_Xdd_tf"]
    ag_data = ag_data.reshape([ag_data.shape[0], ag_data.shape[1], 1]).astype(np.float32)
    u_data = u_data.reshape([u_data.shape[0], u_data.shape[1], 1]).astype(np.float32)
    ut_data = ut_data.reshape([ut_data.shape[0], ut_data.shape[1], 1]).astype(np.float32)
    utt_data = utt_data.reshape([utt_data.shape[0], utt_data.shape[1], 1]).astype(np.float32)
